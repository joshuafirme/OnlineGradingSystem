...
switch ($_POST['form']) {
	...
	case 'savegrade':
			$sql = "UPDATE tbl_studload_qt1 SET 1G = '".$_POST['fG']."' ,2G = '".$_POST['SndG']."',3G = '".$_POST['thrdG']."' ,4G = '".$_POST['fthG']."' WHERE ST_SUBJID = '".$_POST['id']."'";
	...
	case 'loadclass':
			$sql = "SELECT * FROM tbl_loads WHERE userId = '".$_SESSION['userID']."' AND  status = 1";
			$res = mysqli_query($connect, $sql);
			while ($row = mysqli_fetch_array($res)) {
				?>
				<tr>
...
<!-- 					<td style="text-align: center;"><?php
					...
					<td style="text-align: center;"><?php echo ucwords($row['days']."/".$row['time']);  ?></td>
					<td><?php 
					...
					<td style="text-align: center;">
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu">
                             <a class="dropdown-item" href="index.php?url=managegrades&SUBJ_ID=<?php echo $row['SUBJ_ID']; ?>&UID=<?php echo $row['userID']; ?>"><i class="fas fa-edit"></i> Manage Grades</a>
                             <?php 
                             	if ($row['request'] == 'Pending') {
                             		# code...
                             	} else {
                             		?>
							 <a class="dropdown-item" href="javascript:void(0)" onclick="submitGrade('<?php echo $row['id']; ?>');"><i class="fas fa-paper-plane"></i> Submit Grade</a>
                             		<?php
                             	}
                             ?>
                            </div>
