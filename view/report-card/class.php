<?php 
		...
		switch ($_POST['form']) {
			case 'shool_yr':
				...
				while ($row = mysqli_fetch_array($res)) {
					...
					<tr>
						...
						<td style="text-align: center;"><a class="btn btn-info" href="index.php?url=report_card&sy=<?php echo $row['AC_YR'] ?>&term=<?php echo $sem ?>"> View</a></td>
					</tr>
					<?php
			...
			case 'studload':
			$sql = "SELECT * FROM tbl_studload_qt1 WHERE userID = '".$_POST['userID']."' AND ENROLLED_ID = '".$_POST['eid']."'";
			$res = mysqli_query($connect, $sql);
			...
			case 'loadrecords':
				$sql = "SELECT * FROM tbl_erecords WHERE USERID = '".$_POST['student_id']."' AND AC_YR = '".$_POST['sy']."' AND SEMESTER = '".$_POST['term']."'";
				$res = mysqli_query($connect, $sql);
				while ($row = mysqli_fetch_array($res)) {
				?>
				<tr>
